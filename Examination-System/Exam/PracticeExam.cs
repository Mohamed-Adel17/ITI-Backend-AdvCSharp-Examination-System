...
namespace Examination_System.Examination
{
    ...
    {
        public PracticeExam(DateTime date, int duration, int numberOfQuestions, Subject subject) : base(date, duration, numberOfQuestions, subject)
        {
        }
        ...
        {
            return $"Practice Exam for {Subject?.Name} on {Date:yyyy-MM-dd} ({Duration} minutes, {NumberOfQuestions} questions)";
        }

        public override void ShowExam()
        {
            int totalMarks = CalculateTotalMarks();
            Console.WriteLine($"\n-----------------Starting Practice Exam in {Subject.Name}-----------------");
            ...
            foreach (var item in questionsAndAnswers)
            {
                ...
                bool isAnswerCorrect = EvaluateStudentAnswer(item.Key, studentAnswer);
                if (isAnswerCorrect)
                {
                    Console.ForegroundColor = ConsoleColor.DarkGreen;
                    Console.WriteLine("Correct Answer! +" + item.Key.Marks + " Marks");
                    Console.ResetColor();
                }
                else
                {
                    Console.ForegroundColor = ConsoleColor.DarkRed;
                    Console.WriteLine($"Wrong Answer!, The correct answer is: {string.Join(',', item.Key.Answers)}");
                    Console.ResetColor();
                }
